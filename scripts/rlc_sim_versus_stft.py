# -------------------------------------------------------------------------------------------------------------------------------
# NAME: PI-CONTROLLED CUK CONVERTER SIMULATION PROGRAM

# AUTHOR: Alvaro Augusto Volpato

# DESCRIPTION: this program aims to simulate the averaged model of a PI-controlled Cuk converter.
# -------------------------------------------------------------------------------------------------------------------------------
...
v_signal_stft = np.array([abs(y[k,0] + 1j*y[k,2])*cos(omega(t[k])*t[k] + phase(y[k,0] + 1j*y[k,2])) for k in range(len(t))])
...
ax3.plot(t,v_signal_stft,linewidth=0.5 ,color="blue",label="STFT$^{-1}(V_R(t))$")
...
data = np.array([t,v_signal,v_signal_stft]).T

np.savetxt("data_sftft.csv", data, delimiter=",")
pyplot.show()
